...
# Realistic available height = page minus estimated header (~80 pts) minus bottom margin.
# mode='shrink' on KeepInFrame will proportionally scale content that still overflows.
HEADER_EST = 80
KIF_H = PAGE_H - HEADER_EST - BODY_V_MARGIN   # ≈ 740 pts for A4
...
def _styles():
    return dict(
        ...
        s_label=ParagraphStyle(
            ...
            textColor=colors.HexColor(NAVY), spaceBefore=8, spaceAfter=3,  # was 12/5
            textTransform='uppercase', letterSpacing=1.0,
        ),
        s_item=ParagraphStyle(
            'enSItem', fontName='Helvetica', fontSize=8.5,
            textColor=colors.HexColor(DARK), leading=12, spaceAfter=2,
        ),
        section=ParagraphStyle(
            'enSection', fontName='Helvetica-Bold', fontSize=8.5,
            textColor=colors.HexColor(NAVY), textTransform='uppercase',
            letterSpacing=1.2, spaceBefore=7, spaceAfter=1,                # was 10/2
            keepWithNext=True,
        ),
        body=ParagraphStyle(
            'enBody', fontName='Helvetica', fontSize=8.5,
            leading=12, spaceAfter=2, textColor=colors.HexColor(DARK),
        ),
        bold=ParagraphStyle(
            'enBold', fontName='Helvetica-Bold', fontSize=8.5,
        ...
        italic=ParagraphStyle(
            ...
            leading=13, textColor=colors.HexColor(BODY_MUTED), spaceAfter=2,  # was spaceAfter=4
        ),
    )


def _hr():
    return HRFlowable(width='100%', thickness=0.5, color=colors.HexColor(DIVIDER), spaceAfter=3)  # was 4
...
def _build_header_table(user_data, s):
    ...
    if user_data.get('contact_email'):
        ...
        contact_parts.append(f'<a href="mailto:{email}" color="#93c5fd">{email}</a>')
    ...
    if user_data.get('linkedin_url'):
        ...
        contact_parts.append(f'<a href="{linkedin_url}" color="#93c5fd">LinkedIn</a>')
    ...
    header.setStyle(TableStyle([
        ...
        ('TOPPADDING',    (0, 0), (-1, -1), 14),           # was 18 — saves ~8 pts
...
def _build_sidebar(tailored_data, s):
    items = []
...
def _build_main(tailored_data, s):
    ...
    if tailored_data.get('summary'):
        ...
        items.append(Paragraph(tailored_data['summary'], s['body']))
        items.append(Spacer(1, 2))                        # was 4
    ...
    if proj_list:
        ...
        for proj in proj_list:
            ...
            if link:
                items.append(Paragraph(
                    f'<a href="{link}" color="#1d4ed8">{link}</a>', s['small'],
                ))
            if desc:
...
def render_pdf(buffer, user_data, tailored_data):
    ...
    # mode='shrink' ensures content that still overflows is scaled down to fit
    # rather than spilling onto a second page.
    sidebar_kif = KeepInFrame(
        SIDEBAR_W - 14, KIF_H,
        _build_sidebar(tailored_data, s),
    ...
    body.setStyle(TableStyle([
        ...
        ('RIGHTPADDING',  (0, 0), (0,  -1), 10),
        ('TOPPADDING',    (0, 0), (0,  -1), 10),          # was 12
        ('BOTTOMPADDING', (0, 0), (0,  -1), 8),           # was 10
        ('LEFTPADDING',   (1, 0), (1,  -1), 14),
        ('RIGHTPADDING',  (1, 0), (1,  -1), BODY_H_MARGIN),
        ('TOPPADDING',    (1, 0), (1,  -1), 10),          # was 12
        ('BOTTOMPADDING', (1, 0), (1,  -1), 8),           # was 10
    ]))
    story.append(body)
    doc.build(story)
